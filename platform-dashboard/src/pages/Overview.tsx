...
import { Users, MapPin, ShoppingBag, AlertTriangle, ArrowUpRight, TrendingUp } from 'lucide-react'
...
export default function Overview() {
  ...
  return (
    <div className="space-y-10">
      <div>
        <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">Platform Command</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Active Workers" value={workers.length} icon={Users} trend="+4.2%" color="text-emerald-600" />
        <MetricCard label="Tracked Zones" value={zones.length} icon={MapPin} trend="Stable" color="text-slate-600" />
        <MetricCard label="Live Orders" value={totals.liveOrders} icon={ShoppingBag} trend="+12%" color="text-orange-600" />
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <div className="enterprise-panel p-8">
          <div className="flex items-center justify-between mb-8">
             <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">Zone Pressure Matrix</h2>
          ...
          <div className="space-y-4">
            {health.map((item) => (
              <div key={item.zone_id} className="flex items-center justify-between p-4 rounded-lg bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800/60">
                <div>
                   <div className="text-sm font-bold text-slate-900 dark:text-white">Zone {item.zone_id}</div>
                   <div className="text-[10px] text-slate-500 font-medium uppercase tracking-tight mt-0.5">
                      {item.current_orders} / {Math.round(item.baseline_orders)} baseline
                   </div>
                </div>
                <div className="flex items-center gap-4">
                   <div className="text-right">
                      <div className={`text-xs font-black uppercase ${item.status === 'disrupted' ? 'text-rose-500' : 'text-emerald-500'}`}>{item.status}</div>
                      <div className="text-[10px] text-slate-400 mt-0.5">Drop: {Math.round((item.order_drop || 0) * 100)}%</div>
                   </div>
                   <div className={`h-2 w-2 rounded-full ${item.status === 'disrupted' ? 'bg-rose-500 animate-pulse' : 'bg-emerald-500'}`}></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="enterprise-panel p-8">
           <div className="flex items-center justify-between mb-8">
              <h2 className="text-sm font-bold uppercase tracking-widest text-slate-400">Automation Outcome</h2>
           ...
           <div className="grid grid-cols-2 gap-4">
              <div className="p-6 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800/60">
                 <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Payouts Processed</div>
                 <div className="text-2xl font-black text-slate-900 dark:text-white">Rs {Math.round(totals.paid).toLocaleString()}</div>
                 <div className="mt-2 text-[10px] font-bold text-emerald-600 uppercase tracking-tighter">Verified on-chain</div>
              </div>
              <div className="p-6 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800/60">
                 <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Disruptions Caught</div>
                 <div className="text-2xl font-black text-slate-900 dark:text-white">{disruptions.length}</div>
                 <div className="mt-2 text-[10px] font-bold text-orange-600 uppercase tracking-tighter">Averaging 24h cycle</div>
              </div>
           </div>
           ...
           <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800">
              <div className="space-y-3">
                 <div className="flex justify-between items-center text-[11px]">
                    <span className="text-slate-500 font-medium tracking-tight">Active Coverage</span>
                    <span className="text-slate-900 dark:text-white font-bold">100% Guaranteed</span>
                 </div>
...
function MetricCard({ label, value, icon: Icon, trend, color }: { label: string; value: string | number; icon: any; trend: string; color: string }) {
  return (
    <div className="enterprise-panel p-6 flex flex-col justify-between min-h-[140px]">
      ...
      <div>
        <div className="text-3xl font-black text-slate-900 dark:text-white mt-4">{value}</div>
