...
import { Activity, ShieldAlert, BarChart, MapPin, Wind, CloudRain, Zap } from 'lucide-react'
    ...
    <div className="space-y-10">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {zones.map((zone) => {
          ...
          return (
            <div key={zone.zone_id} className={`enterprise-panel relative group overflow-hidden ${isDisrupted ? 'border-rose-500/40 shadow-[0_0_20px_rgba(244,63,94,0.1)]' : ''}`}>
              <div className="p-8">
                <div className="flex items-start justify-between mb-8">
                  <div>
                    <div className="flex items-center gap-2">
                       <MapPin className="h-4 w-4 text-slate-400" />
                       <h2 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">{zone.name}, {zone.city}</h2>
                    </div>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1.5">{zone.state} • Risk Rating {zone.risk_rating}</p>
                  </div>
                  <div className={`px-2 py-1 rounded-sm text-[9px] font-black uppercase tracking-widest border ${
                    isDisrupted 
                ...
                <div className="grid grid-cols-2 gap-8">
                   <div>
                      ...
                      <div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Live Workers</div>
                   </div>
                   <div>
                      <div className={`text-2xl font-black tracking-tight ${drop >= 30 ? 'text-rose-500' : 'text-emerald-500'}`}>{drop}%</div>
                      <div className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">Volume Drop</div>
                   </div>
                </div>
              ...
              <div className="h-1 w-full bg-slate-100 dark:bg-slate-800">
                 <div className={`h-full transition-all duration-1000 ${isDisrupted ? 'bg-rose-500 w-full' : 'bg-emerald-400 w-full opacity-30'}`}></div>
              </div>
            </div>
          )
        })}
...
function SignalBadge({ icon: Icon, label, active }: { icon: any, label: string, active?: boolean }) {
  return (
    <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded border transition-colors ${
      active 
