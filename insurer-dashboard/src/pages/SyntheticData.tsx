...
export default function SyntheticData() {
  ...
  return (
    <PageShell
      ...
      description="Seed deterministic demo data for the full InDel pipeline. Rapidly initialize realistic insurer operations."
    >
            ...
            <div className="grid gap-8 md:grid-cols-2">
              <label className="space-y-3">
                ...
                <input
                  ...
                  className="w-full rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 py-4 text-xl font-black text-slate-900 dark:text-white outline-none focus:border-orange-600 transition-none"
                />
              </label>
              ...
              <label className="space-y-3">
                ...
                <input
                  ...
                  className="w-full rounded border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 py-4 text-lg font-bold text-slate-900 dark:text-white outline-none focus:border-orange-600 transition-none"
                />
              </label>
            </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                {scenarios.map((item) => (
                  ...
                  >
                    ...
                    <p className={`mt-2 text-[11px] leading-relaxed ${scenario === item.value ? 'text-orange-100' : 'text-slate-500 dark:text-slate-500'}`}>{item.note}</p>
                  </button>
                ))}
              </div>
            </div>
        ...
        <Panel title="Projections" subtitle="Projected impact of the current simulation.">
          <div className="grid gap-6">
            ...
            <div className="p-6 rounded border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-[11px] leading-relaxed text-slate-500">
               ...
               Targeting vector <span className="text-orange-600 font-bold uppercase">{scenario.replace('_', ' ')}</span> to validate systemic resilience against correlated loss events.
            </div>
          </div>
        </Panel>
      </div>
