...
func SetupWorkerRoutes(router *gin.Engine) {
	...
	v1.GET("/worker/batches/delivered", worker.GetDeliveredBatches)
	v1.POST("/worker/batches/:batch_id/accept", worker.AcceptBatch)
	v1.PUT("/worker/batches/:batch_id/deliver", worker.DeliverBatch)
	...
	v1.POST("/worker/fetch-verification/send-code", worker.SendVerificationCode)
	v1.POST("/worker/fetch-verification/verify", worker.VerifyFetchCode)
